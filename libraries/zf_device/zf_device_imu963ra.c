...
static void imu963ra_connect_mag (uint8 addr, uint8 reg)
{
    ...
    // TODO: 尝试将 0x6C 改为 0x4C（与写/读寄存器时一致），0x6C 可能设置了导致停止更新的位
    imu963ra_write_acc_gyro_register(IMU963RA_MASTER_CONFIG, 0x4C);             // 开启上拉 I2C主机使能
}   
...
static uint8 imu963ra_acc_gyro_self_check (void)
{
    ...
    while(0x6B != dat)                                                          // 判断 ID 是否正确
    {
        if(IMU963RA_TIMEOUT_COUNT < timeout_count ++)
        {
            printf("Self-check failed! Last WHO_AM_I: 0x%02X\n", dat);
            return_state = 1;
            break;
        }
...
void imu963ra_get_mag (void)
{
    ...
    // TODO: 临时添加 printf 帮助定位问题，查看是否是因为 status 一直没有 0x01 导致不更新数据
    // 增加限流防止 printf 导致单片机卡死
    static uint16 print_count = 0;
    if(print_count++ % 50 == 0) {
        printf("STATUS_MASTER: 0x%02X\r\n", temp_status);
    }
...
//        printf("Mag raw data: dat[0]=0x%02X, dat[1]=0x%02X, dat[2]=0x%02X, dat[3]=0x%02X, dat[4]=0x%02X, dat[5]=0x%02X\r\n",
//                     dat[0], dat[1], dat[2], dat[3], dat[4], dat[5]);
//              printf("Mag converted data: X=%d, Y=%d, Z=%d\r\n",
